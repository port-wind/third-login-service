...
基于 Cloudflare Workers 的轻量级第三方登录认证服务，支持 Google、Apple Sign In、Phone（手机号）等多平台登录。
...
- 🌐 **多平台** - Google、Apple、Phone（手机号）（可扩展 LINE、Facebook 等）
...
- [App 登录接口](./API_APP_LOGIN.md) - App SDK 登录接口文档（Google/Apple）
- [Phone 登录接口](./API_PHONE_LOGIN.md) - Phone 登录接口文档（手机号验证码）
...
**Firebase 手机登录：** 除客户端 Firebase 工程外，Worker 必须配置 **`FIREBASE_PROJECT_ID`**（与客户端 `projectId` 一致），否则 `/auth/phone` 及 `/auth/verify` 中的手机 Firebase Token 无法通过校验。说明见 [API_PHONE_LOGIN.md](./API_PHONE_LOGIN.md)。

详见仓库内各环境 `wrangler.*.toml` 注释与 [Phone 登录接口](./API_PHONE_LOGIN.md)。
...
├── API_PHONE_LOGIN.md       # Phone（Firebase 短信）登录文档
├── API_H5_LOGIN.md          # H5 登录接口文档
├── API_VERIFY_TOKEN.md      # 后台验证接口文档
├── DEPLOYMENT.md            # 部署指南
├── INTEGRATION_GUIDE.md     # 集成指南
├── test-sign.js             # 测试脚本
├── wrangler.toml            # Wrangler 默认入口（pnpm 脚本使用 wrangler.dev.toml / wrangler.prod.toml）
├── wrangler.dev.toml        # 测试环境配置（pnpm dev / deploy 默认）
