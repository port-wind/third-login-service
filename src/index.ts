...
app.get('/test', (c) => {
  ...
  const firebaseAuthDomain = c.env.FIREBASE_AUTH_DOMAIN || ''
...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .phone-form { display: grid; gap: 12px; margin-top: 12px; }
    .form-group { display: flex; gap: 8px; align-items: stretch; }
    .form-input { flex: 1; padding: 10px 14px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; font-family: inherit; }
    .form-input:focus { outline: none; border-color: #667eea; }
    .btn-primary { background: #667eea; color: white; padding: 10px 20px; border: none; border-radius: 6px; font-size: 14px; font-weight: 500; cursor: pointer; white-space: nowrap; }
...
<body>
  <div class="container">
    ...
    <div class="section">
      <div class="section-title">📱 Phone Auth 测试 <span class="badge">Firebase</span></div>
      <div class="info">使用 Firebase Phone Authentication 测试手机号登录。需要先配置 Firebase 项目。</div>
      <div class="phone-form">
        <div class="form-group">
          <input type="tel" id="phone-number" class="form-input" placeholder="+8613800138000" />
          <button class="btn-primary" onclick="sendPhoneCode()" id="send-code-btn">发送验证码</button>
        </div>
        <div class="hint">请输入国际格式手机号，例如：+8613800138000</div>
        <div id="recaptcha-container"></div>
        <div class="form-group" id="verify-group" style="display: none;">
          <input type="text" id="verify-code" class="form-input" placeholder="输入验证码" />
          <button class="btn-primary" onclick="verifyPhoneCode()">验证登录</button>
        </div>
      </div>
  ...
  <script>
    ...
    // 初始化 Firebase
    try {
      if (!firebase.apps.length && FIREBASE_CONFIG.apiKey) {
        ...
        'size': 'normal',
        'callback': (response) => {
          console.log('reCAPTCHA 已验证');
        },
        'expired-callback': () => {
          console.log('reCAPTCHA 已过期，请重新验证');
        }
      });
        ...
        if (phoneResult) {
          phoneResult.className = 'result show error';
          phoneResult.innerHTML = '<div class="result-title">⚠️ Firebase 未配置</div><div>请在 wrangler.dev.toml（或当前环境的 wrangler 配置）中配置 FIREBASE_API_KEY、FIREBASE_AUTH_DOMAIN、FIREBASE_PROJECT_ID</div>';
        }
      }
    } catch (error) {
      ...
      if (phoneResult) {
        phoneResult.className = 'result show error';
        phoneResult.innerHTML = '<div class="result-title">⚠️ Firebase 初始化失败</div><div>' + error.message + '</div>';
      }
    }
    ...
    async function sendPhoneCode() {
      ...
      if (!phoneNumber) {
        ...
        resultDiv.innerHTML = '<div class="result-title">❌ 请输入手机号</div>';
        return;
      }
      
      if (!phoneNumber.startsWith('+')) {
        resultDiv.className = 'result show error';
        resultDiv.innerHTML = '<div class="result-title">❌ 手机号格式错误</div><div>请使用国际格式，例如：+8613800138000</div>';
        return;
      }
      ...
      resultDiv.innerHTML = '<div class="result-title">📤 正在发送验证码...</div>';
      ...
      try {
        ...
        resultDiv.innerHTML = '<div class="result-title">✅ 验证码已发送</div><div>请查收短信并输入验证码</div>';
        
        document.getElementById('verify-group').style.display = 'flex';
        document.getElementById('verify-code').focus();
      } catch (error) {
        console.error('发送验证码失败:', error);
        resultDiv.className = 'result show error';
        resultDiv.innerHTML = '<div class="result-title">❌ 发送失败</div><div>' + error.message + '</div>';
    ...
    async function verifyPhoneCode() {
      ...
      if (!code) {
        resultDiv.className = 'result show error';
        resultDiv.innerHTML = '<div class="result-title">❌ 请输入验证码</div>';
        return;
      }
      
      resultDiv.className = 'result show';
      resultDiv.innerHTML = '<div class="result-title">🔐 正在验证...</div>';
      ...
      try {
        ...
        resultDiv.className = 'result show success';
        resultDiv.innerHTML = '<div class="result-title">✅ 验证成功！</div><div><strong>手机号:</strong> ' + result.user.phoneNumber + '</div><div><strong>Firebase UID:</strong> ' + result.user.uid + '</div><div>正在调用登录服务...</div>';
        
        // 调用我们的登录服务
        await callPhoneLoginService(idToken, result.user.phoneNumber);
      } catch (error) {
        console.error('验证失败:', error);
        resultDiv.className = 'result show error';
        resultDiv.innerHTML = '<div class="result-title">❌ 验证失败</div><div>' + error.message + '</div>';
      }
    }
    
    async function callPhoneLoginService(idToken, phoneNumber) {
      const resultDiv = document.getElementById('phone-result');
      
      try {
        // 生成认证 JWT（注意：生产环境应该在后端生成！）
        const authJWT = await generateTestAuthJWT();
        ...
        if (result.success) {
          resultDiv.className = 'result show success';
          resultDiv.innerHTML = '<div class="result-title">🎉 登录成功！</div>' +
            '<div><strong>User ID:</strong> ' + result.data.user_id + '</div>' +
            '<div><strong>手机号:</strong> ' + result.data.phone_number + '</div>' +
            '<div><strong>是否新用户:</strong> ' + (result.data.is_new ? '是' : '否') + '</div>' +
            '<div><strong>JWT Token:</strong></div><pre>' + result.data.jwt + '</pre>';
        } else {
          resultDiv.className = 'result show error';
          resultDiv.innerHTML = '<div class="result-title">❌ 登录失败</div>' +
            '<div><strong>错误:</strong> ' + result.message + '</div>';
        }
      } catch (error) {
        console.error('调用登录服务失败:', error);
        resultDiv.className = 'result show error';
        resultDiv.innerHTML = '<div class="result-title">❌ 登录服务调用失败</div><div>' + error.message + '</div>';
      }
    }
    
    // 生成测试用的认证 JWT（仅用于测试！）
    async function generateTestAuthJWT() {
      // 注意：这里使用了测试配置，生产环境必须在后端生成！
