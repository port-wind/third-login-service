...
| **支持的 Token** | ✅ 系统 JWT (我们签发的)<br>✅ Firebase Token（含 Google / Apple / **Phone**）<br>✅ Google ID Token |

> **Firebase 手机登录：** 若请求体中的 Token 为 **Firebase Phone Auth** 签发的 ID Token，登录服务必须在 Worker 环境变量中配置 **`FIREBASE_PROJECT_ID`**，且与客户端使用的 Firebase 项目一致，否则返回校验失败。详见 [Phone 登录接口](./API_PHONE_LOGIN.md) 中「登录服务（Cloudflare Worker）」一节。
...
**说明：** 
- `email`：Google/Apple 登录时返回
- `phone_number`：Phone 登录时返回（国际格式，如 +8613800138000）
...
**说明：** 
- `email` / `phone_number`：根据登录方式返回（Phone 登录时有 `phone_number`，Google/Apple 登录时有 `email`）
...
| `provider` | string | 登录平台 | `google` / `apple` / `phone` |
| `provider_uid` | string | **第三方平台的用户 ID** | Phone 登录时为手机号 |
| `app_id` | string | 应用 ID | - |
| `email` | string \| null | **用户邮箱** | Google/Apple 登录时返回，Phone 登录时为 `null` |
| `phone_number` | string \| null | **用户手机号**（国际格式） | **Phone 登录时返回**，Google/Apple 登录时为 `null` |
...
|-----------|--------|------|---------|
| **Firebase Token** | Firebase | Google / Apple / **Phone** 登录 | `iss: https://securetoken.google.com/{项目ID}`；手机登录另需 Worker 配置 `FIREBASE_PROJECT_ID` |
